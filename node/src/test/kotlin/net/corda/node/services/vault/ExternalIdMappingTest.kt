...
class ExternalIdMappingTest {
    ...
    fun `states can be queried by external ID`() {
        ...
        val dummyStateForExternalId = createDummyState(listOf(anonymousMe))
        // Create a state with a public key not assigned to the new external ID.
        val anonymousMeTwo = freshKey()
        createDummyState(listOf(anonymousMeTwo))
        ...
        val dummyStateForExternalIdTwo = createDummyState(listOf(anonymousMeThree))
    ...
    fun `Two states can be mapped to a single externalId`() {
        ...
        // This query should return two states!
        val result = database.transaction {
            val externalId = builder { VaultSchemaV1.ExtIdToPubKeyView::externalId.`in`(listOf(id)) }
            val queryCriteria = QueryCriteria.VaultCustomQueryCriteria(externalId)
            vaultService.queryBy<DummyState>(queryCriteria).states
        }
    ...
    fun `One state can be mapped to multiple externalIds`() {
        ...
        // This query should return one states!
        val result = database.transaction {
            val externalId = builder { VaultSchemaV1.ExtIdToPubKeyView::externalId.`in`(listOf(idOne, idTwo)) }
            val queryCriteria = QueryCriteria.VaultCustomQueryCriteria(externalId)
            vaultService.queryBy<DummyState>(queryCriteria).states
        }
