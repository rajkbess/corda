...
data class Amount<T : Any>(val quantity: Long, val displayTokenSize: BigDecimal, val token: T) : Comparable<Amount<T>> {
     ...
     * Returns 'partitions', separate Amount objects which sum to the same quantity as this Amount
...
class AmountTransfer<T : Any, P : Any>(val quantityDelta: Long,
     ...
     * Returns a copy of the original list, except that funds needed to cover the exchange
