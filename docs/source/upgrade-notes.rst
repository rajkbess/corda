...
Let's take a very simple flow that finalises a transaction without the involvement of a counterpart flow:
...
This needs to be annotated with ``InitiatingFlow`` and a ``FlowSession`` to the participant of the transaction must be
...
The responder flow, which will automatically run on the other participants' nodes, just has to call ``ReceiveFinalityFlow``
...
This is no longer necessary with ``ReceiveFinalityFlow`` and can be removed.
