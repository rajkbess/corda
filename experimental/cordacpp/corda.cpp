...
CompositeTypeGuard::CompositeTypeGuard(::proton::codec::decoder &decoder, const ::proton::symbol &expected, int num_fields) : decoder(decoder),  sym(expected) {
    if (decoder.next_type() != proton::DESCRIBED)
        throw std::invalid_argument(msg() << "Expected a described element, but got " << decoder.next_type());
    proton::codec::start start;
    decoder >> start;
    proton::symbol s;
    decoder >> s;
    if (s != expected)
        throw std::invalid_argument(msg() << "Expected descriptor " << expected << " but got " << s);
    if (num_fields > 0) {
        pop_second = true;
        decoder >> block;
        if (block.size != num_fields)
            throw std::invalid_argument(msg() << "Expected " << num_fields << " fields but got " << block.size);
    }
}
